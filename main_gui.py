...
# Worker Thread (Agar GUI Tidak Freeze/Crash)
...
class SemanticSegApp(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.btn_load.setStyleSheet("background-color: #3498db; color: white; font-size: 20px; font-weight: bold; border-radius: 5px;")
        self.btn_load.clicked.connect(self.load_image)
        btn_layout.addWidget(self.btn_load)

        self.btn_process = QPushButton("Mulai Segmentasi")
        self.btn_process.setFixedHeight(50)
        self.btn_process.setStyleSheet("background-color: #e74c3c; color: white; font-size: 20px; font-weight: bold; border-radius: 5px;")
    ...
    def load_image(self):
        file_name, _ = QFileDialog.getOpenFileName(self, "Pilih Gambar", "", "Images (*.png *.jpg *.jpeg *.bmp)")
        ...
        if file_name:
            try:
                ...
                self.btn_process.setStyleSheet("background-color: #2ecc71; color: white; font-size: 14px; font-weight: bold; border-radius: 5px;")
    ...
    def start_segmentation(self):
        ...
        self.lbl_status.setText("Sedang memproses... (Mungkin butuh beberapa detik untuk download model pertama kali)")
        self.btn_process.setEnabled(False)
    ...
    def display_image(self, pil_image, qlabel_widget):
        ...
        scaled_pixmap = pixmap.scaled(qlabel_widget.size(), Qt.AspectRatioMode.KeepAspectRatio, Qt.TransformationMode.SmoothTransformation)
    ...
    def update_legend(self, predictions_array):
        ...
        for class_id in unique_classes:
            if class_id == 0: # Abaikan background
                continue
            ...
            legend_item_frame.setStyleSheet("background-color: #3e526a; border-radius: 3px; padding: 3px;")
            ...
            color_box.setStyleSheet(f"background-color: rgb({color_rgb[0]},{color_rgb[1]},{color_rgb[2]}); border: 1px solid white;")
        ...
        if not detected_objects:
            no_obj_label = QLabel("Tidak ada objek yang dikenal model terdeteksi.")
            no_obj_label.setStyleSheet("color: #bdc3c7; font-size: 11px;")
            self.legend_layout.addWidget(no_obj_label)
        else:
